      ...
      if (m_random->GetValue () > snrPer.per)
        {
          ...
          //Set values in the objects
          snrTag.Set(signalNoise.signal - signalNoise.noise);
          noiseTag.Set(signalNoise.noise);
          rxPowerTag.Set(signalNoise.signal);

          // Check tags in packet and add them if not present
          if (! packet->PeekPacketTag (snrTag))
          {
            packet->AddPacketTag (snrTag);
            NS_LOG_UNCOND("Add SNR Tag with value :" << snrTag.Get());
          }

          if (! packet->PeekPacketTag (noiseTag))
          {
            packet->AddPacketTag (noiseTag);
            NS_LOG_UNCOND("Add noise Tag with value :" << noiseTag.Get());
          }

          if (! packet->PeekPacketTag (rxPowerTag))
          {
            packet->AddPacketTag (rxPowerTag);
            NS_LOG_UNCOND("Add received signal power Tag with value :" << rxPowerTag.Get());
          }
          // *********  BROADCAST TREE PROTOCOL TAGS  *********** //
